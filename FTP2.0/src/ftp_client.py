    ...
    while True:
        ...
        # 显示当前目录内容
            ...
            if data.get("file_name"):
                ...
                for item in data.get("file_name"):
                    print(item)

            if data.get("folder_name"):

                print("子目录:".center(40,'-'))
                for item in data.get("folder_name"):
                    print(item)
        ...
        #删除文件
            ...
            # 第一个列表元素是操作；第二个列表元素是文件名
            filename = cmd_list[1]
            msg_data = {"action": "del", "filename": filename}

            s.sendall(bytes(json.dumps(msg_data),encoding='utf-8'))

            confirm=s.recv(2048)

            if confirm.decode()=='100' or confirm.decode()=='200':
                ipt=input("请确认是否删除(y/n)")
                if ipt=='y':
                    s.sendall(bytes(ipt,encoding='utf-8'))
                    if s.recv(1024).decode()=='done':
                        print("成功删除")
                        continue
                    else:
                        continue
                ...
                print("你所指定的文件或者目录不存在！")

                continue

        elif task_type== 'help':
            print("欢迎使用FTP 模拟系统，您可以使用以下功能".center(40,'*'))
            msg="""
            查看当前目录      dir
            创建目录         mkdir  dir1\dir2
            切换目录         cd directory_name
            删除目录或文件    del file_name
            上传文件         put file_name
            下载文件         get file_name
            显示当前目录      pwd
            帮助            help
